...
window.McuMgrFlutter = {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (const img of images) {
          ...
          const eraseSlot = slot === 0 ? 1 : slot * 2 + 1
          this._log(`Erasing slot ${eraseSlot} before upload...`)
          try {
            await mgr.cmdImageErase(eraseSlot)
            this._log(`Slot ${eraseSlot} erased.`)
          } catch (e) {
            this._log(`Erase slot ${eraseSlot} failed (ignoring): ${e}`)
          }
        ...
        try {
          ...
          this._log(`Image hash valid: ${info.hashValid}, hash: ${info.hash}`)
          if (info.hash) {
            const hashBytes = new Uint8Array(info.hash.match(/.{1,2}/g).map((byte) => parseInt(byte, 16)))
            const confirmResp = await mgr.cmdImageConfirm(hashBytes)
            this._log(`Confirm response: ${JSON.stringify(confirmResp)}`)
          }
        } catch (e) {
          this._log(`Image confirm failed: ${e}`)
          throw e
        }

        emitState('eraseAppSettings')
        this._log('Sending reset...')
        try {
          const resetDisconnectPromise = new Promise((resolveReset) => {
            mgr.onDisconnect(() => {
              this._log('Device disconnected after reset.')
              resolveReset()
            })
            setTimeout(() => {
          ...
          mgr._userRequestedDisconnect = true
          await mgr.cmdReset()
  ...
  getCachedDevices: function () {
    if (!window._mcumgrDeviceCache) return []
    return Array.from(window._mcumgrDeviceCache.values()).map((d) => ({
      id: d.id,
      name: d.name ?? null,
      watchingAdvertisements: d.watchingAdvertisements ?? false,
    }))
  },

  clearCachedDevices: function () {
    if (window._mcumgrDeviceCache) window._mcumgrDeviceCache.clear()
  },
}
