...
class MCUManager {
  ...
  _processMessage(message) {
    ...
    if (group === MGMT_GROUP_ID_IMAGE && id === IMG_MGMT_ID_STATE) {
      if (this._imageStateCallback) {
        this._imageStateCallback(data)
        this._imageStateCallback = null
      }
      return
    }
    if (group === MGMT_GROUP_ID_IMAGE && id === IMG_MGMT_ID_ERASE) {
      if (this._imageEraseCallback) {
        this._imageEraseCallback(data)
        this._imageEraseCallback = null
      }
      return
    }
  ...
  cmdImageErase(slot = 1) {
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        this._imageEraseCallback = null
        reject(new Error(`cmdImageErase slot ${slot} timeout`))
      }, 30000)
      this._imageEraseCallback = (data) => {
        clearTimeout(timeout)
        if (data.rc && data.rc !== 0) {
          reject(new Error(`Image erase failed with rc=${data.rc}`))
        } else {
          resolve(data)
        }
      }
      this._sendMessage(MGMT_OP_WRITE, MGMT_GROUP_ID_IMAGE, IMG_MGMT_ID_ERASE, { slot }).catch((e) => {
        clearTimeout(timeout)
        this._imageEraseCallback = null
        reject(e)
      })
  ...
  cmdImageTest(hash) {
    return this._sendImageStateCommand(MGMT_OP_WRITE, { hash, confirm: false })
  }
  cmdImageConfirm(hash) {
    return this._sendImageStateCommand(MGMT_OP_WRITE, { hash, confirm: true })
  }
  _sendImageStateCommand(op, data) {
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        this._imageStateCallback = null
        reject(new Error('cmdImageState timeout'))
      }, 10000)
      this._imageStateCallback = (response) => {
        clearTimeout(timeout)
        if (response.rc && response.rc !== 0) {
          reject(new Error(`Image state command failed with rc=${response.rc}`))
        } else {
          resolve(response)
        }
      }
      this._sendMessage(op, MGMT_GROUP_ID_IMAGE, IMG_MGMT_ID_STATE, data).catch((e) => {
        clearTimeout(timeout)
        this._imageStateCallback = null
        reject(e)
      })
  ...
  async _uploadNext() {
    ...
    if (this._uploadOffset === 0) {
      ...
      if (this._uploadSlot > 0) message.image = this._uploadSlot
    }
