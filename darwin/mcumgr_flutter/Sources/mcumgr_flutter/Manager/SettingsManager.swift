...
final class SettingsManager {
    ...
    private let useByteStringEncoding: Bool
    ...
    init(transport: McuMgrBleTransport, padTo4Bytes: Bool = false, encodeValueToCBOR: Bool = false, useByteStringEncoding: Bool = true, precisionMode: String = "auto", logStreamHandler: StreamHandler? = nil) {
        ...
        self.useByteStringEncoding = useByteStringEncoding
    ...
    func writeSetting(key: String, value: Any, result: @escaping FlutterResult) {
        do {
            ...
            sendLogToDart("Key and value to write: \(key), value: \(valueBytes), useByteStringEncoding: \(useByteStringEncoding)")

            if useByteStringEncoding {
                // Byte string encoding: value → [UInt8] → CBOR bstr
    ...
    private func handleWriteResponse(response: McuMgrResponse?, error: Error?, result: @escaping FlutterResult) {
        if let error = error {
            result(FlutterError(code: Self.errorCode,
                                message: error.localizedDescription,
                                details: nil))
...
private enum SettingsCommandID: UInt8 {
    case readWrite = 0
}
